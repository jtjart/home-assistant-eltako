...
class EltakoEntity(Entity):
    ...
    def __init__(
        ...
        self._attr_dev_name = config_helpers.get_device_name(
            dev_name, dev_id, self.general_settings
        )
    ...
    def _get_identifier(
        ...
        return (
            f"{DOMAIN}_gw{gateway.dev_id}_{config_helpers.format_address(dev_id)}{description_key}".replace(
                " ", "_"
            )
            .replace("-", "_")
            .lower()
    ...
    def device_info(self) -> DeviceInfo:
        ...
        return DeviceInfo(
            identifiers={(DOMAIN, config_helpers.format_address(self.dev_id))},
            name=self.dev_name,
    ...
    async def async_added_to_hass(self) -> None:
        ...
        event_id = config_helpers.get_bus_event_type(
            self.gateway.base_id, SIGNAL_RECEIVE_MESSAGE
        )
    ...
    def send_message(self, msg: ESP2Message):
        ...
        event_id = config_helpers.get_bus_event_type(
            self.gateway.base_id, SIGNAL_SEND_MESSAGE
        )
...
def get_entity_from_hass(
    hass: HomeAssistant, domain: Platform, dev_id: AddressExpression
) -> bool:
