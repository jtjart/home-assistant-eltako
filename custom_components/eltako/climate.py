    ...
    if platform in config:
        for entity_config in config[platform]:
            try:
                ...
                thermostat = config_helpers.get_device_conf(
                    entity_config, CONF_ROOM_THERMOSTAT
                )
                ...
                if CONF_COOLING_MODE in config.keys():
                    ...
                    _LOGGER.debug("Read cooling sender config")
                ...
                if dev_conf.eep in [A5_10_06]:
                    ...
                    climate_entity = ClimateController(
                        platform,
                        gateway,
                        dev_conf.id,
                        dev_conf.name,
                        ...
                        dev_conf.get(CONF_MIN_TARGET_TEMPERATURE),
                        dev_conf.get(CONF_MAX_TARGET_TEMPERATURE),
                        thermostat,
                        cooling_switch,
                        cooling_sender,
                    )
                    ...
                    if cooling_switch is not None:
                        event_id = config_helpers.get_bus_event_type(
                            ...
                            EVENT_BUTTON_PRESSED,
                            cooling_switch.id,
                            config_helpers.convert_button_pos_from_hex_to_str(
                                cooling_switch.get(CONF_SWITCH_BUTTON)
                            ),
                        )
                        _LOGGER.debug(
                            "Subscribe for listening to cooling switch events: %s",
                            event_id,
                        )
                        hass.bus.async_listen(
                            event_id, climate_entity.async_handle_event
                        )
...
class ClimateController(EltakoEntity, ClimateEntity, RestoreEntity):
    ...
    COOLING_SWITCH_SIGNAL_FREQUENCY_IN_MIN: int = (
        15  # FTS14EM signals are repeated every 15min
    )
            ...
            try:
                ...
                if self.thermostat is None:
                    await self._async_send_command(
                        self._actuator_mode, self.target_temperature
                    )
    ...
    async def async_set_temperature(self, **kwargs) -> None:
        ...
        if self._actuator_mode != None and self.current_temperature > 0:
            ...
            _LOGGER.debug(
                "[%s] default state of actor was not yet transferred",
                self.dev_id,
            )

    async def _async_send_command(
        self, mode: A5_10_06.Heater_Mode, target_temp: float
    ) -> None:
    ...
    def _send_command(self, mode: A5_10_06.Heater_Mode, target_temp: float) -> None:
        ...
        if self.current_temperature and self.target_temperature:
            _LOGGER.debug(
                "[%s] Send status update: current temp: %s, mode: %s",
                self.dev_id,
                target_temp,
                mode,
            )
    ...
    async def _async_check_if_cooling_is_activated(self) -> None:
        ...
        _LOGGER.debug("[%s] {%s} mode is activated", self.dev_id, new_mode)
    ...
    def value_changed(self, msg: ESP2Message) -> None:
        ...
        if msg.address == climate_address:
            _LOGGER.debug("Change state triggered by actuator: %s", self.dev_id)
        ...
        if self.thermostat:
            ...
            if msg.address == thermostat_address:
                _LOGGER.debug(
                    "[%s] Change state triggered by thermostat: %s",
                    self.dev_id,
                    self.thermostat.id,
                )
                self.change_temperature_values(msg)
    ...
    def change_temperature_values(self, msg: ESP2Message) -> None:
        try:
            ...
            _LOGGER.warning("[%s] Could not decode message: %s", self.dev_id, str(e))
            return
        ...
        if msg.org == 0x07 and self.dev_eep in [A5_10_06]:
            ...
            if decoded.mode != A5_10_06.Heater_Mode.OFF:
                ...
                self._attr_target_temperature = (
                    round(2 * decoded.target_temperature, 0) / 2
                )
