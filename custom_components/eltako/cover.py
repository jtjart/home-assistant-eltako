            ...
            try:
                ...
                entities.append(
                    EltakoCover(
                        ...
                        dev_conf.get(CONF_TIME_CLOSES),
                        dev_conf.get(CONF_TIME_OPENS),
                        dev_conf.get(CONF_TIME_TILTS),
                    )
                )
...
class EltakoCover(EltakoEntity, CoverEntity):
                ...
                if decoded.direction == 0x01:  # up
                    ...
                    if self._time_tilts is not None:
                        self._attr_current_cover_tilt_position = min(
                            ...
                            + int(decoded.time / self._time_tilts * 100.0),
                            100,
                        )
    ...
    def set_cover_tilt_position(self, **kwargs: Any) -> None:
        ...
        if tilt_position > self._attr_current_cover_tilt_position:
            ...
            sleeptime = min(
                (
                    (tilt_position - self._attr_current_cover_tilt_position)
                    / 100.0
                    * self._time_tilts
                    / 10.0
                ),
