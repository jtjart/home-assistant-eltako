    ...
    for platform_id in [Platform.BINARY_SENSOR, Platform.SENSOR]:
        if platform_id in config:
            for entity_config in config[platform_id]:
                try:
                    dev_conf = config_helpers.DeviceConf(
                        entity_config, [CONF_DEVICE_CLASS, CONF_INVERT_SIGNAL]
                    )
                        ...
                        if dev_conf.eep == A5_30_03:
                            ...
                            entities.append(
                                EltakoBinarySensor(
                                    ...
                                    name,
                                    dev_conf.eep,
                                    dev_conf.get(CONF_DEVICE_CLASS),
                                    dev_conf.get(CONF_INVERT_SIGNAL),
...
class EltakoBinarySensor(AbstractBinarySensor):
    ...
    def __init__(
        ...
        if description:
            self.entity_description = EntityDescription(
                key=description.key, name=description.name
            )
    ...
    def value_changed(self, msg: ESP2Message):
        ...
        if self.dev_eep in [F6_02_01, F6_02_02]:
            # _LOGGER.debug("[Binary Sensor][%s] Received msg for processing eep %s telegram.", b2s(self.dev_id[0]), self.dev_eep.eep_string)
            pressed_buttons = []
            ...
            event_id = config_helpers.get_bus_event_type(
                self.gateway.dev_id,
                EVENT_BUTTON_PRESSED,
                AddressExpression((msg.address, None)),
            )
            event_data = {
                "id": event_id,
            ...
            if self.description_key == "0":
                ...
                raise Exception(
                    "[%s %s] EEP %s Unknown description key for A5-30-03",
                    Platform.BINARY_SENSOR,
                    str(self.dev_id),
                    A5_30_03.eep_string,
                )
