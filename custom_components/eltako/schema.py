        ...
        vol.Schema(
            {
                ...
                vol.Required(CONF_SENDER): _get_sender_schema(
                    CONF_SENDER_EEP_SUPPORTED
                ),
                vol.Optional(CONF_NAME, default=DEFAULT_NAME): cv.string,
            }
        ...
        vol.Schema(
            {
                ...
                vol.Optional(
                    CONF_METER_TARIFFS, default=DEFAULT_METER_TARIFFS
                ): vol.All(
                    cv.ensure_list, [vol.All(vol.Coerce(int), vol.Range(min=1, max=16))]
                ),
                vol.Optional(CONF_DEVICE_CLASS): BINARY_SENSOR_DEVICE_CLASSES_SCHEMA,
            }
        ),
    ...
    CONF_COOLING_MODE_SCHEMA = vol.Schema(
        {
            vol.Required(
                CONF_SENSOR
            ): vol.Schema(  # detects if heater is switch globally into cooling mode
                {
                    vol.Required(CONF_ID): cv.matches_regex(CONF_ID_REGEX),
            ...
            vol.Optional(
                CONF_SENDER
            ): vol.Schema(  # sends frequently a signal to stay in cooling mode if detect by cooling-mode-sensor
                {
                    vol.Required(CONF_ID): cv.matches_regex(CONF_ID_REGEX),
                    vol.Required(CONF_EEP): vol.In(
                        [F6_02_01.eep_string, F6_02_02.eep_string]
                    ),
                    vol.Optional(
                        CONF_NAME, default=DEFAULT_COOLING_SENDER_NAME
                    ): cv.string,
                    vol.Optional(CONF_GATEWAY_ID, default=None): cv.matches_regex(
                        CONF_ID_REGEX
                    ),
                }
            ),
        ...
        vol.Schema(
            {
                ...
                vol.Optional(CONF_NAME, default=DEFAULT_NAME): cv.string,
                vol.Optional(CONF_ROOM_THERMOSTAT): _get_sender_schema(
                    CONF_CLIMATE_SENDER_EEP
                ),  # physical thermostat like FUTH
                vol.Optional(
                    CONF_COOLING_MODE
                ): CONF_COOLING_MODE_SCHEMA,  # if not provided cooling is not supported
            }
        ),
    )
        ...
        DOMAIN: vol.Schema(
            {
                ...
                vol.Optional(CONF_GATEWAY): vol.All(
                    cv.ensure_list, [GatewaySchema.ENTITY_SCHEMA]
                ),
            }
        )
    },
