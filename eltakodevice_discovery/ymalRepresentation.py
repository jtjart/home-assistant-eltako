...
EEP_MAPPING = [
    {
        "hw-type": "FTS14EM",
        CONF_EEP: "F6-02-01",
        CONF_TYPE: Platform.BINARY_SENSOR,
    ...
    },
    {
        ...
        CONF_EEP: "A5-12-02",
        CONF_TYPE: Platform.SENSOR,
        "description": "Automated meter reading - electricity",
        "address_count": 1,
    },
    {
        "hw-type": "F3Z14D",
        CONF_EEP: "A5-13-01",
        CONF_TYPE: Platform.SENSOR,
        "description": "Automated meter reading - gas",
        "address_count": 1,
    },
    {
        "hw-type": "F3Z14D",
        CONF_EEP: "A5-12-03",
        CONF_TYPE: Platform.SENSOR,
        "description": "Automated meter reading - water",
        "address_count": 1,
    },
    {
        "hw-type": "FUD14",
        CONF_EEP: "A5-38-08",
        "sender_eep": "A5-38-08",
        CONF_TYPE: Platform.LIGHT,
        "description": "Central command - gateway",
        "address_count": 1,
    },
    {
        "hw-type": "FSR14_1x",
    ...
    },
    {
        ...
        CONF_EEP: "G5-3F-7F",
        "sender_eep": "H5-3F-7F",
        CONF_TYPE: Platform.COVER,
        "description": "Eltako cover",
        "address_count": 2,
    },
    {
        "hw-type": "FAE14SSR",
        CONF_EEP: "A5-10-06",
        "sender_eep": "A5-10-06",
        CONF_TYPE: Platform.CLIMATE,
        "description": "Eltako heating/cooling",
        "address_count": 2,
    },
]
...
ORG_MAPPING = {
    ...
    6: {
        "Telegram": "1BS",
        "RORG": "D5",
        CONF_NAME: "1 Byte Communication",
        CONF_TYPE: Platform.SENSOR,
        CONF_EEP: "D5-??-??",
    },
    7: {
        "Telegram": "4BS",
        "RORG": "A5",
        CONF_NAME: "4 Byte Communication",
        CONF_TYPE: Platform.SENSOR,
        CONF_EEP: "A5-??-??",
    },
}
...
class HaConfig:
    ...
    def get_detected_sensor_by_id(self, id: str) -> dict:
        if id not in self.detected_sensors.keys():
            self.detected_sensors[id] = {CONF_ID: id}
    ...
    def find_sensors(self, dev_id: int, in_func_group: int) -> [SensorInfo]:
        ...
        for s in self.collected_sensor_list:
            if (
                int.from_bytes(s.dev_adr, "big") == dev_id
                and s.in_func_group == in_func_group
            ):
                result.append(s)
        return result
    ...
    def add_or_get_sensor(self, sensor_id: str, device_id: str, dev_type: str) -> dict:
        ...
        if sensor_id not in self.detected_sensors.keys():
            logging.info(
                colored(
                    f"Add sensor: address: {sensor_id} from device {device_id} and device type: {dev_type}",
                    "yellow",
                )
            )
            sensor = {
    ...
    def filter_out_base_address(self, sensor_id: bytes) -> bool:
        ...
        return (
            self.sender_base_address > sensor_id_int
            or self.sender_base_address + 128 < sensor_id_int
        )
    ...
    async def add_device(self, device: BusObject):
        ...
        if info != None:
            ...
            for i in range(0, info["address_count"]):
                ...
                if (
                    "sender_eep" in info
                ):  # info[CONF_TYPE] in ['light', 'switch', 'cover']:
                    dev_obj["sender"] = {
                ...
                if info[CONF_TYPE] == Platform.CLIMATE:
                    ...
                    if thermostat:
                        ...
                        dev_obj[CONF_ROOM_THERMOSTAT][CONF_ID] = b2s(
                            thermostat.sensor_id
                        )
                        dev_obj[CONF_ROOM_THERMOSTAT][CONF_EEP] = (
                            A5_10_06.eep_string
                        )  # TODO: derive EEP from switch/sensor function

                        # add thermostat into sensor
                        sensor = self.add_or_get_sensor(
                            b2s(thermostat.sensor_id), dev_id_str, device_type
                        )
                ...
                logging.info(
                    colored(
                        f"Add device {info[CONF_TYPE]}: id: {dev_obj[CONF_ID]}, eep: {dev_obj[CONF_EEP]}, name: {dev_obj[CONF_NAME]}",
                        "yellow",
                    )
                )
        ...
        if type(msg) == Regular1BSMessage:
            try:
                data = (
                    b"\xa5\x5a"
                    + msg.body[:-1]
                    + (sum(msg.body[:-1]) % 256).to_bytes(2, "big")
                )
        ...
        if type(msg) in SENSOR_MESSAGE_TYPES:
            ...
            if hasattr(msg, "outgoing"):
                ...
                if address not in self.detected_sensors.keys():
                    ...
                    comment = (
                        f"Sensor Type: {sensor_type}, Derived from Msg Type: {msg_type}"
                    )
            ...
            if type(msg) == EltakoDiscoveryRequest and msg.address == 127:
                logging.info(
                    colored(
                        "Wait for incoming sensor singals. After you have recorded all your sensor singals press Ctrl+c to exist and store the configuration file.",
                        "red",
                        attrs=["bold"],
                    )
                )
    ...
    def generate_config(self) -> str:
        ...
        out += f"  {CONF_GATEWAY}:\n"
        out += f"  - {CONF_ID}: 1\n"
        fam14 = GatewayDeviceType.EltakoFAM14.value
        fgw14usb = GatewayDeviceType.EltakoFGW14USB.value
    ...
    def config_section_to_string(
        self, config, is_list: bool, space_count: int = 0
    ) -> str:
        out = ""
        spaces = space_count * " " + "        "
        S = "-" if is_list else " "
